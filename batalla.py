    ...
    def get_user_guess(self):
        while True:  # loop until user provides valid input (row=A-J, col=1-10, and that its an empty cell on board
            try:
                ...
                user = input("{}, Selecciona Fila y Columna (e.j. B8): ".format(self.name))  # get row and column from user
...
def main():
    try:
        ...
        ##nuevo sonido
        pygame.mixer.init()
        shot_sound = pygame.mixer.Sound("disparo.wav")  # Reemplaza "disparo.wav" con el nombre de tu archivo de sonido
        ...
        if args.server:
            s.bind(("0.0.0.0", port))
            s.listen(1)
            print('Esperando un oponente que se conecte {}:{}'.format(get_ip_address(), port))
            s, addr = s.accept()
            print('Recivio coneccion: {}:{}'.format(addr[0], addr[1]))
            my_turn = False
        else:
        ...
        my_name = input("Ingresa tu nombre: ")  # get my name
        ...
        while True:  # main game loop
            if my_turn:  # if it is my turn
                ...
                if hit:  # if I hit one of the opponent's ships
                    msg = "Adivinaste {}{} - HUNDISTE UN BARCO".format(row, col)
                    ...
                    msg = "Adivinaste mal {}{} - FALLASTE".format(row, col)
                ...
                print("esperando a {} que haga un movimiento".format(opponent_name))  # prompt waiting for opponent
                ...
                if p.check_if_hit(row, col):  # if opponent hit one of my ships
                    msg = "{} Adivinaste {}{} - HUNDISTE UN BARCO".format(opponent_name, row, col)
                    ...
                    msg = "{} Adivinaste mal {}{} - fallaste".format(opponent_name, row, col)
        ...
        if p.my_ships_destroyed > p.opponent_ships_destroyed:  # if opponent destroyed more ships than me
            print("{} ganó!".format(opponent_name))  # declare opponent as the winner
        else:  # if I destroyed more ships than opponent
            print("{} ganó!".format(my_name))  # declare me as the winner
        ##########
    except KeyboardInterrupt:
        print("\nBye!")
